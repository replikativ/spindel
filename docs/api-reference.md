...
| `(signal initial-value)` / `(signal ctx initial-value)` | Create a reactive signal with deterministic ID. The 2-arity form takes an explicit execution-context (e.g. `(signal runtime [])`) and is what top-level `def`s in the examples use. |
...
|----------------|-------------|
| `(signal initial-value)` / `(signal ctx initial-value)` | Create signal with deterministic ID (macro). 2-arity form takes an explicit execution-context, used at top-level `def`s. |
...
see [Incremental Collections](incremental.md) for the algebra contract.
