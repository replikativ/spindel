(ns is.simm.spindel.state.signal
...
;; See is.simm.spindel.incremental.interval for the implementation.
;;
     ...
     ([initial-value]
      (let [source-loc {:file *file*
        ...
        `(let [ctx# (rtc/current-execution-context)
               id# (is.simm.spindel.runtime.addressing/next-address! ctx# "signal" ~source-loc)]
