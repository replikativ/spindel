(ns is.simm.spindel.dom.foreach
  ...
  **Spin Support (Not Yet Implemented):**

  Spins inside render-fn are not currently supported. The render-fn must
  return a vnode synchronously. Supporting spins in render-fns would require
  CPS-transforming the iteration, which is a design decision for the future.
  ...
  **Integration with parent elements:**
  ...
  (:require [is.simm.spindel.dom.fragment :as frag]
...
(defn for-each*
  ...
  Returns: KeyedFragment with :items and :deltas"
  ...
  (let [;; Compute address for this ifor-each
        ...
        _ (when (and (d/deltaable? source)
                     (not (instance? #?(:clj is.simm.spindel.incremental.interval.Interval
    ...
    (let [source-keys (set (map key-fn source-new))
      ...
      (let [;; Process each delta to produce output delta and updated cache
            result
            (reduce
              (fn [{:keys [by-key order deltas]} delta]
                (case (:delta delta)
                  :add
                  (let [item (:value delta)
                        k (key-fn item)
                        ;; Render with keyed context
                        vnode (addr/with-keyed-context my-addr k
                                (fn [] (render-fn item)))
                  ...
                  (let [;; :remove deltas may have item in :old-value (from DeltaableVector)
                    ...
                    (if (>= idx 0)
                      ...
                      {:by-key by-key :order order :deltas deltas}))

                  :update
                  (let [item (:value delta)
                        k (key-fn item)
                        old-vnode (get by-key k)
                        ;; Re-render with keyed context
                        new-vnode (addr/with-keyed-context my-addr k
                                    (fn [] (render-fn item)))
                        idx (.indexOf order k)]
                  ...
                  {:by-key by-key :order order :deltas deltas}))
              {:by-key prev-by-key :order prev-order :deltas []}
              source-deltas)

            new-by-key (:by-key result)
            new-order (:order result)
            out-deltas (:deltas result)

            ;; Build items in order, clearing stale deltas from cached vnodes
            ;; Cached vnodes may have :deltas from their original element* reconciliation
            ;; which would cause duplicate DOM children if not cleared
            items (mapv #(clear-stale-deltas (get new-by-key %)) new-order)]
      ...
      (let [items-with-keys
            ...
            by-key (into {} (map (juxt :key :vnode) items-with-keys))
            order (mapv :key items-with-keys)
            items (mapv :vnode items-with-keys)
