(ns is.simm.spindel.runtime.context
  ...
  (:require [is.simm.spindel.runtime.impl.simple :as simple]
            [is.simm.spindel.runtime.protocols :as rtp]
            [is.simm.spindel.runtime.scheduler :as sched]
            [is.simm.spindel.runtime.state-backend :as backend]
...
(def incognito-write-handlers
  ...
  (atom {'is.simm.spindel.runtime.node_types.SpinNode
         (fn [node] (into {} node))

         'is.simm.spindel.runtime.node_types.SignalNode
         (fn [node] (into {} node))

         'is.simm.spindel.spin.result.Result
         (fn [r] (into {} r))}))
  ...
  (cas-state! [_ path old-val new-val]
    ...
    (case (backend/backend-type backend)
      ...
      (let [success? (volatile! false)
            state-atom (:state-atom backend)]
        (swap! state-atom
               (fn [ctx]
                 (let [current-val (get-in ctx path)]
                   (if (= current-val old-val)
                     (do
                       (vreset! success? true)
                       (assoc-in ctx path new-val))
                     ctx))))
        @success?)
      ...
      (let [is-fork-local? (and (seq path)
        ...
        (if is-fork-local?
          ...
          (let [success? (volatile! false)
                overlay-atom (:overlay-atom backend)]
            (swap! overlay-atom
                   (fn [ov]
                     (let [current-val (get-in ov path)]
                       (if (= current-val old-val)
                         (do
                           (vreset! success? true)
   ...
   (defmacro with-rebuild-context
    ...
    Example:
     ...
     `(let [rebuild-ctx# (prepare-rebuild-context
        ...
        (require 'is.simm.spindel.runtime.core)
