(ns is.simm.spindel.effects.core
  ...
  - **Call-form symbols**: Effects are invoked by registering call-form symbols
    (e.g., is.simm.spindel.spin/await) with handlers and small arg adapters.
...
(defn register-effect-by-symbol!
  ...
  - sym: fully-qualified symbol used in code (e.g., 'is.simm.spindel.spin/await)
...
(defn dispatch-symbol-call
  ...
  - sym: the call-form symbol (e.g., 'is.simm.spindel.spin/await)
  ...
  (let [{:keys [handler adapter-fn adapter]} (get @effect-syntax-registry sym)]
    ...
    (let [adapter-fn (or adapter-fn
      ...
      (when (= sym 'is.simm.spindel.effects.await/await)
