(ns is.simm.spindel.distributed.macros
  ...
  #?(:cljs (:require-macros [is.simm.spindel.distributed.macros])))
               ...
               (let [local-sym (symbol (name ns-sym))]
                 {:def `(defn ~local-sym [arg-map#]
                          ...
                          (let [context-id# (:__context-id__ arg-map#)
                            ...
                            (if ctx#
                              (binding [is.simm.spindel.runtime.core/*execution-context* ctx#]
                                (dist/spin->chan
                                  (is.simm.spindel.spin.cps/spin
                                    ~@remote-body)))
