...
;; Map of block-id → live DOM element. Populated by the `:ref` callback
;; on each rendered block (see `render-block`). Keeps an O(1) handle
;; without ever calling querySelector. The focus-spin reads this map
;; to focus the currently-focused block.
(defonce block-elements (atom {}))
...
(defn- register-content-el!
  "Ref callback: stash the contenteditable element in `block-elements`
   on mount, drop it on unmount. Reconciliation reuses the same element,
   so this fires once per block lifetime — exactly the discipline `:ref`
   is built for."
  [block-id el]
  (if el
    (swap! block-elements assoc block-id el)
    (swap! block-elements dissoc block-id)))
...
(defn render-block
  ...
  (let [bid (:id block)]
    (el/div {:class (str "block"
      ...
      ;; Content area — :ref hooks the live DOM element into block-elements
      ;; so the focus-spin can focus it without querySelector.
      (el/div {:class "block-content"
               :contenteditable "true"
               :data-id (str bid)
               :data-action "edit"
               :ref (fn [el] (register-content-el! bid el))}
        (:content block)))))
...
(defonce focus-spin-handle (atom nil))

(defn- focus-content-el!
  "Focus the given contenteditable element and place the caret at the
   end of its content. No-op when the element is already focused."
  [el]
...
(defn make-focus-spin
  "Reactive spin that focuses the currently-focused block.

   Reads the focused block-id from `blocks-sig` and pulls the live DOM
   element out of `block-elements` (populated by `:ref` on each rendered
   block). The `queueMicrotask` deferral is the ordering trick: when
   the user action mutates blocks-signal, both this spin and the
   rendering spin become dirty. The rendering spin's resolve commits
   DOM synchronously. By scheduling the focus on a microtask, we run
   AFTER all spin observers (and therefore after `render-effect`)
   have settled, so `block-elements` is up to date and the new element
   exists in the DOM. No setTimeout, no querySelector, no sentinel
   vnode."
  [blocks-sig]
  (spin
    (let [blocks-iv (track blocks-sig)
          focused-block (first (filter :focused (iv/get-new blocks-iv)))]
      (when-let [bid (:id focused-block)]
        (js/queueMicrotask
          (fn []
            (when-let [el (get @block-elements bid)]
              (focus-content-el! el)))))
      nil)))
  ...
  (let [container (js/document.getElementById "editor-container")
    ...
    (binding [ec/*execution-context* runtime]
      (let [app-spin (make-app-spin blocks-signal)
            focus-spin (make-focus-spin blocks-signal)]
        (reset! render-handle
                (render/render-spin! container app-spin discharge))
        ;; Kick the focus-spin once. After this it self-schedules via
        ;; (track blocks-signal); no render-spin! needed because it has
        ;; no vnode output — it's a pure side-effect spin.
        (reset! focus-spin-handle focus-spin)
        (focus-spin identity identity)))
