(ns is.simm.spindel.partial-cps-integration-test
...
(defn async-sleep
  ...
  Returns: An async thunk (fn [resolve reject] ...) that:
    1. Schedules a timer
    2. Fires a :deferred-delivery event when timer completes
    3. Resumes the async computation with the value"
  [ms value]
  (fn [resolve _reject]
    (let [runtime (rtc/current-execution-context)]
      (if runtime
        ;; In spindel context - use event system for proper integration
        (let [deferred-fn (fn [v]
                            ;; Called by process-event! with proper bindings
                            (cont/resume resolve v))]
          ;; Schedule timer to fire deferred-delivery event
          (future
            (Thread/sleep ms)
            (binding [rtc/*execution-context* runtime rtc/*execution-context* runtime]
