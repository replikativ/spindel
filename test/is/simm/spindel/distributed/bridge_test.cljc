(ns is.simm.spindel.distributed.bridge-test
...
(deftest test-spin->chan-nil-value
  (testing "spin->chan handles nil values via sentinel"
    (let [t (spin nil)
      ...
      (is (= :is.simm.spindel.distributed.core/nil-value @result-atom)))))
...
#?(:clj
   (deftest test-chan->spin-nil
     (testing "chan->spin handles nil via sentinel"
       (let [ch (chan 1)
             t (dist/chan->spin ch)
             _ (put! ch :is.simm.spindel.distributed.core/nil-value)
             result @(spin (await t))]
         (is (nil? result))))))
